...
async def process_and_upload_image(file_bytes: bytes, task_id: str) -> list[str]:
    ...
    original_image_bytes, rotated_image_bytes, gray_image_bytes, scaled_image_bytes = (
        await asyncio.gather(
            ...
            asyncio.to_thread(
                convert_image_to_bytes, image=gray_image, image_format=original_format
            ),
            asyncio.to_thread(
                convert_image_to_bytes, image=scaled_image, image_format=original_format
            ),
        )
    )
    ...
    await asyncio.gather(
        asyncio.to_thread(
            ...
            file_name=f"{task_id}_original.{format_extension}",
        ),
        asyncio.to_thread(
            upload_to_s3,
            image=rotated_image_bytes,
            file_name=f"{task_id}_rotated.{format_extension}",
        ),
        asyncio.to_thread(
            upload_to_s3,
            image=gray_image_bytes,
            file_name=f"{task_id}_gray.{format_extension}",
        ),
        asyncio.to_thread(
            upload_to_s3,
            image=scaled_image_bytes,
            file_name=f"{task_id}_scaled.{format_extension}",
        ),
    )

    return [
        f"{task_id}_original.{format_extension}",
